...
void NetworkManagerServer::HandlePacketFromNewClient( InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress )
{
	...
	if(  packetType == kHelloCC )
	{
		...
		string secret = StringUtils::GetCommandLineArg( 2 );

		//verify login token
		string data = name + avatar + accountId + secret;
		...
		if (str.str() != token)
		{
			LOG( "Invalid login token given by connecting client. Given %s, generated %s", str.str().c_str(), token.c_str() );
			return;
		}
