...
public:
	...
	const	InputState&		GetInputState()		const	{ return mInputState; }
			...
			const	MoveList&				GetProcessedMoveList() const		{ return mProcessedMoveList; }
					MoveList&				GetProcessedMoveList()				{ return mProcessedMoveList; }

			void							SetLastHyperYarnShotTime(float time){ mLastHyperYarnShot = time; }
			float							GetLastHyperYarnShotTime()	const	{ return mLastHyperYarnShot; }
...
private:
	...
	float			mLastHyperYarnShot;
	float			mTimeToRespawn;

	MoveList		mUnprocessedMoveList;
	MoveList		mProcessedMoveList;
