...
public:
	...
	const	Move&		AddMove(const Move& inMove);
			bool		AddMoveIfNew( const Move& inMove );

	deque<Move>			RemovedProcessedMoves( float inLastMoveProcessedOnServerTimestamp );

	float				GetLastMoveTimestamp()	const	{ return mLastMoveTimestamp; } 

	const Move&			GetLatestMove()			const	{ return mMoves.back(); }
	deque<Move>			GetMovesSince(float timestamp) const;
