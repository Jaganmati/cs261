...
void RoboCatClient::ProcessInput(float inDeltaTime, const InputState& inInputState)
{
	RoboCat::ProcessInput(inDeltaTime, inInputState);

	if (mIsHyperShooting && World::sInstance->GetHyperYarn().empty())
	{
		std::shared_ptr<HyperYarn> hyperYarn = std::shared_ptr<HyperYarn>(dynamic_cast<HyperYarn*>(new HyperYarnClient));
		Vector3 ray(GetForwardVector() * 100 /* TODO: Cast to the edge of the window */);
		hyperYarn->setPoints(GetLocation(), GetLocation() + ray);
		World::sInstance->AddHyperYarn(hyperYarn);

		for (auto& obj : World::sInstance->GetGameObjects())
		{
			RoboCat* cat = obj->GetAsCat();
			if (cat && cat->GetPlayerId() != NetworkManagerClient::sInstance->GetPlayerId() && hyperYarn->Intersects(std::static_pointer_cast<RoboCat>(obj)))
			{
				hyperYarn->setColor(1, 0, 0);
			}
		}
	}
}
