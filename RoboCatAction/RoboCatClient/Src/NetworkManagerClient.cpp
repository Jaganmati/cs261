...
void NetworkManagerClient::StaticInit( const SocketAddress& inServerAddress, const string& inAccountId, const string& inName, const string& inAvatar, const string& inToken )
{
	sInstance = new NetworkManagerClient();
	return sInstance->Init( inServerAddress, inAccountId, inName, inAvatar, inToken );
}

void NetworkManagerClient::Init( const SocketAddress& inServerAddress, const string& inAccountId, const string& inName, const string& inAvatar, const string& inToken)
{
...
void NetworkManagerClient::ProcessPacket( InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress )
{
	...
	case kHyperYarnCC:
		if (mState == NCS_Welcomed)
		{
			...
			inInputStream.Read( intersects );
			std::shared_ptr<HyperYarn> const& yarn = World::sInstance->GetHyperYarn().front();

			if (intersects)
				yarn->setColor(1, 1, 1);
			else yarn->setColor(0, 0, 0);
		}
		break;
	}
}
...
void NetworkManagerClient::SendHelloPacket()
{
	...
	helloPacket.Write( mAccountId );
	helloPacket.Write( mName );
	helloPacket.Write( mAvatar );
	helloPacket.Write( mLoginToken );
